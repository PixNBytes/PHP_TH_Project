...
if ($_SERVER["REQUEST_METHOD"] == "POST") {	//If suggest method is post we execute, but if not then just shows the suggest.
	$name = trim(filter_input(INPUT_POST, "name", FILTER_SANITIZE_STRING));		//best to trim here rather than in the validation
	$email = trim(filter_input(INPUT_POST, "email", FILTER_SANITIZE_EMAIL));					// Prevents any typ of attack
	$category = trim(filter_input(INPUT_POST, "category", FILTER_SANITIZE_STRING));
	$title = trim(filter_input(INPUT_POST, "title", FILTER_SANITIZE_STRING));
	$format = trim(filter_input(INPUT_POST, "format", FILTER_SANITIZE_STRING));
	$genre = trim(filter_input(INPUT_POST, "genre", FILTER_SANITIZE_STRING));
	$year = trim(filter_input(INPUT_POST, "year", FILTER_SANITIZE_STRING));
	$details = trim(filter_input(INPUT_POST, "details", FILTER_SANITIZE_SPECIAL_CHARS));  //takes any html and converts to special characters

	if ($name == "" || $email == "" || $category == "" || $title == "") {
		$error_message = "Please fill in the required fields: Name, Email, Category, and Title";
	}

	if (!isset($error_message) && $POST["address2"] != "") {
	...
	if(!isset($error_message)) {
		...
		$mail->setFrom($email, $name);	//set whom the email is from from our above variables
		$mail->addAddress('jason840@mail.ca', 'Jason');     // Add a recipient -- Name is optional
		// $mail->addReplyTo('info@example.com', 'Information');
		// $mail->addCC('cc@example.com');
		// $mail->addBCC('bcc@example.com');		We don't need these lines as it will default reply to sender

		// $mail->addAttachment('/var/tmp/file.tar.gz');         // Add attachments  Don't need here
		// $mail->addAttachment('/tmp/image.jpg', 'new.jpg');    // Optional name
		$mail->isHTML(false);                                  // Set email format to HTML  -- False sets to plain text

		$mail->Subject = 'Personal Media Library Suggestion from ' . $name;
		$mail->Body    = $email_body;
		// $mail->AltBody = 'This is the body in plain text for non-HTML mail clients';     Don't need cuz we are sending in plain text

		if($mail->send()) {
			header("location:suggest.php?status=thanks");
		}
	    $error_message = 'Message could not be sent.';
	    $error_message .= 'Mailer Error: ' . $mail->ErrorInfo;  //Displays the property with the error message
	}
}
...
<div class="section page">
	...
	<div class="wrapper">
		...
		<?php if (isset($_GET["status"]) && $_GET["status"] == "thanks") {
			...
			echo "<p class= 'thanks-message'>Thanks for the email!</br>I&rsquo;ll check out your suggestion shortly!</p>";
		} else {
			
			if (isset($error_message)) {
				echo "<p class='message'>" . $error_message . "</p>";
			} else {
				echo "<p class='suggest-item-sentence'>If you think there is something I&rsquo;m missing, let me know! </br> Complete the form to send me an email.</p>";
		...
		<form method="post" action="suggest.php">
			...
			<table>
				...
				<tr>
					...
					<td><select name="format" id="format">
						...
						<optgroup label="Books">
							...
							<option value="Hardcover"<?php if (isset($format) && $format == "Hardcover") { echo " selected"; } ?>>Hardcover</option>
				...
				<tr style="display: none"> <!-- hide the field with CSS so user doensn't fill in -->
					<th><label for="address2">Address2</label></th>		<!-- Spam honeypot field ... make look like a real field -->
					<td><input type="text" name="address2" id="address2">
						<p>Please leave this field blank</p> <!--Tries to prevent accidental input by users using screen-readers ect -->
					</td>
				</tr>
			</table>
